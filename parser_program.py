...
new_arg=""
new_binary=""
def get_format_size(opcode_length,binary_number,current_arg,number_of_arg):
...
null_binary=bin(0)[2:];
while True:
    padded_reg=0;# false at beginning
    ...
    print(split_line)
    print("The length of the file is " + str(len(file_line)))
    ...
    for i in range(len(reg_and_imm)):
        arg=reg_and_imm[i]
        number_of_arg=len(reg_and_imm)
        print("CURR arg is "+ arg)
        #print("i is "+str(i))
        #print(bin(int(arg)))
        binary_arg=bin(int(arg))[2:]
        if(opcode_size==11 and number_of_arg==2 and padded_reg==0):
            arg_array.append(null_binary.zfill(7)) #pad to 7 bits for unused register
            padded_reg=1;
        binary_arg= get_format_size(opcode_size,binary_arg,i,number_of_arg)
